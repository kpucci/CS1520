...
function getCats() {
	...
	makeReq("GET", "/cats", 200, showCategories);
}
...
function addPurchase(){
    ...
    if(verifyInputs(purchaseName,purchaseAmount,"purchase")){
        ...
        makeReq("POST", "/purchases", 201, getCats, data);
...
function addCategory(){
    ...
    if(verifyInputs(categoryName,categoryBudget,"category")){
        ...
        makeReq("POST", "/cats", 201, getCats, data);
...
function showCategories(responseText){
    ...
    createUncategorizedSummary(purch_totals[0],all_purch_total);
...
function filterByDate(month,year){
    function filterPurchases(purch){
        ...
        var purchYear = purchDate[0];   // Get purchase year
        var purchMonth = purchDate[1];  // Get purchase month
        return (purchYear == year && purchMonth == month);
    }
...
function saveCat(){
    ...
    if(verifyInputs(name,newBudget,"category")){
        ...
        // Make AJAX request to post new category to category list
        makeReq("PUT", "/cats/"+name, 201, getCats, data);
    }
    document.getElementById("edit-cat-modal").style.display="none";
}
...
function deleteCat(){
    var name = document.getElementById("edit-cat-header").value;
    makeReq("DELETE", "/cats/"+name, 204, getCats);
    document.getElementById("edit-cat-modal").style.display="none";
}
